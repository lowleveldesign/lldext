...
extern "C" void CALLBACK DebugExtensionNotify(ULONG notify, ULONG64 argument) {
    ...
    switch (notify) {
        // A debugging session is active. The session may not necessarily be
        // suspended.
...
HRESULT CALLBACK help(IDebugClient* dbgclient, PCSTR args) {
    ...
    if (SUCCEEDED(dbgclient->QueryInterface(__uuidof(IDebugControl),
        (void**)&dbgctrl))) {
        dbgctrl->Output(DEBUG_OUTPUT_NORMAL,
            "==============================================================\n");
        dbgctrl->Output(DEBUG_OUTPUT_NORMAL,
            "  LLD v1.3 - Copyright 2023 Sebastian Solnica\n");
        dbgctrl->Output(DEBUG_OUTPUT_NORMAL,
            "==============================================================\n\n");
